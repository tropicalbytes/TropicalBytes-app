 ...
 *     -> validate business rules (combo exists, ids are known, etc.)
...
/** YYYY-MM-DD, not in the past, not further out than MAX_FUTURE_DATE_DAYS. */
...
function validateAndNormalize(requestType, raw) {
  ...
  if (requestType === REQUEST_TYPES.SUBSCRIPTION) {
    const durationId = raw.durationId;
    const mealPreference = raw.mealPreference;
    const foodPreference = raw.foodPreference;
    const quantity = raw.quantity;
    const startDate = raw.startDate;

    check(isNonEmptyString(durationId) && Object.prototype.hasOwnProperty.call(GENERATED_ALLOWLIST.DURATIONS, durationId), "durationId");
    ...
    const combo = GENERATED_ALLOWLIST.COMBOS[comboIdFor(foodPreference, mealPreference)];
    check(!!combo, "combo");
    if (errors.length > 0) return { ok: false, errors: errors };

    const duration = GENERATED_ALLOWLIST.DURATIONS[durationId];
    const price = calculatePrice(combo.pricePerWeek, duration.weeks, duration.discountPercent);

    return {
      ok: true,
      data: Object.assign({}, base, {
        selectedPlan: combo.label,
        ...
        estimatedTotal: "\u20B9" + price.toLocaleString("en-IN"), // authoritative, server-calculated
...
function comboIdFor(foodType, mealTime) {
  const ids = Object.keys(GENERATED_ALLOWLIST.COMBOS);
  for (let i = 0; i < ids.length; i++) {
    const c = GENERATED_ALLOWLIST.COMBOS[ids[i]];
    if (c.foodType === foodType && c.mealTime === mealTime) return ids[i];
  }
  return null;
}

function calculatePrice(pricePerWeek, weeks, discountPercent) {
  const gross = pricePerWeek * weeks;
